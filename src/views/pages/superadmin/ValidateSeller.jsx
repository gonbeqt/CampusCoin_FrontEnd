...
const ValidateSeller = () => {
    ...
    const fetchsellers = async () => {
      ...
      try {
        ...
        if (result?.success && Array.isArray(result.users)) {
          const onlyPendingsellers = result.users
            .filter(user => user.role === 'seller' && user.accountStatus === 'pending')
            .map((user) => ({
              id: user._id,
              ...
              documents: [
                ...(user.documents?.birCertificate
                  ? [
                    {
                      id: user.documents.birCertificate,
                ...
                ...(user.documents?.businessPermit
                  ? [
                    {
                      id: user.documents.businessPermit,
          ...
          setsellers(onlyPendingsellers)
        } else {
          setMessage({
            type: 'error',
            text: result?.error || 'Failed to load seller data',
          })
        }
      } catch (error) {
  ...
  const handleApprove = async (sellerId) => {
    ...
    try {
      const token = localStorage.getItem('authToken')
      const result = await ValidationController.approveUser(sellerId, token)
      ...
      if (result.success) {
        ...
        setMessage({ type: 'error', text: result.error || 'Failed to approve seller' })
      }
    } catch (error) {
      console.error('Approve error:', error)
      setMessage({ type: 'error', text: 'An error occurred while approving the seller' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const performReject = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: 'success', text: 'seller rejected' })
        setsellers(sellers.filter((seller) => seller.id !== rejectTargetId))
        if (selectedseller?.id === rejectTargetId) setSelectedseller(null)
        setShowRejectModal(false)
        setRejectTargetId(null)
        setRejectReason('')
      } else {
        setMessage({ type: 'error', text: result.error || 'Failed to reject seller' })
      }
    } catch (error) {
      console.error('Reject error:', error)
      setMessage({ type: 'error', text: 'An error occurred while rejecting the seller' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const filteredsellers = (sellers || []).filter((seller) => {
    if (searchQuery) {
      ...
      if (!seller.name.toLowerCase().includes(query) && !seller.email.toLowerCase().includes(query)) {
        return false
      }
    }
    if (filter === 'bir') {
      return seller.documents.some((doc) => doc.name.toLowerCase().includes('bir'))
    } else if (filter === 'bp') {
      return seller.documents.some((doc) => doc.name.toLowerCase().includes('bp'))
    }
    return true
  })
  ...
  return (
    <div className="pt-16 md:ml-64">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Seller Validation</h1>
          <p className="text-gray-600">Review and validate seller accounts with BIR or Business Permit </p>
        </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredsellers.map((seller) => (
                      <tr
                        key={seller.id}
                        className={`cursor-pointer ${selectedseller?.id === seller.id ? 'bg-blue-50' : 'hover:bg-gray-50'
                          }`}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-gray-500">
                            {new Date(seller.dateApplied).toLocaleDateString()}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-gray-500">
                            {seller.documents.length} document(s)
                          </span>
                        </td>
          ...
          {selectedseller ? (
            <div className="bg-white rounded-lg shadow p-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-base font-medium">{selectedseller.name}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Email</p>
                  <p className="text-base">{selectedseller.email}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Date Applied</p>
                  <p className="text-base">{new Date(selectedseller.dateApplied).toLocaleDateString()}</p>
                </div>
                <div>
            ...
            <div className="bg-white rounded-lg shadow p-5 h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-500">Select an seller to view their details</p>
              </div>
            </div>
          )}
        </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-2">Reject Seller</h3>
