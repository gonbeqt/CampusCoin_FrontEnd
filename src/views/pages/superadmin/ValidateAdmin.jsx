...
const ValidateAdmin = () => {
    ...
    const fetchAdmins = async () => {
      ...
      try {
        ...
        if (result?.success && Array.isArray(result.users)) {
          const onlyPendingAdmins = result.users
            .filter(user => user.role === 'admin' && user.accountStatus === 'pending')
            .map(user => ({
              id: user._id,
              ...
              documents: user.documents
                ? [{
                  id: user.documents.teachingCredential,
                  name: user.documents.credentialType || 'Document',
                }]
                : [],
            }))

          setAdmins(onlyPendingAdmins)
        } else {
          setMessage({
            type: 'error',
  ...
  try {
    const token = localStorage.getItem('authToken')
    const result = await ValidationController.approveUser(adminId, token)
    ...
    if (result.success) {
      ...
      setMessage({ type: 'error', text: result.error || 'Failed to approve admin' })
    }
  } catch (error) {
    console.error('Approve error:', error)
    setMessage({ type: 'error', text: 'An error occurred while approving the admin' })
  } finally {
    setIsLoading(false)
  }
}
  ...
  const handleReject = async (adminId) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'An error occurred while rejecting the admin' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="pt-16 md:ml-64">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Admin Validation</h1>
          <p className="text-gray-600">Review and validate admin accounts with TCP or LPT certification</p>
        </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredAdmins.map((admin) => (
                      <tr
                        key={admin.id}
                        className={`cursor-pointer ${selectedAdmin?.id === admin.id ? 'bg-blue-50' : 'hover:bg-gray-50'
                          }`}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-gray-500">
                            {new Date(admin.dateApplied).toLocaleDateString()}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-gray-500">
                            {admin.documents.length} document(s)
                          </span>
                        </td>
          ...
          {selectedAdmin ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedAdmin.documents.map((doc) => (
                      <div
                        key={doc.id}
            ...
            <div className="bg-white rounded-lg shadow p-5 h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-500">Select an admin to view their details</p>
              </div>
            </div>
          )}
        </div>
